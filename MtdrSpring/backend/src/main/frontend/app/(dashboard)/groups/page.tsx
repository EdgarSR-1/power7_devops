...
function NewGroupModal({ users, isLoading, onClose, onCreate }: NewGroupModalProps) {
...
export default function GroupsPage() {
  ...
  const handleCreateGroup = async (name: string, userIds: number[]) => {
    try {
      ...
      setError("Error creando el grupo o agregando integrantes. Revisa que el backend esté disponible.");
    } finally {
  ...
  const handleDeleteGroup = async (
    ...
    if (!window.confirm(`¿Eliminar "${group.title}"?`)) {
      return;
    }

    try {
      setDeletingGroupId(group.id);
      const deleted = await deleteTaskGroup(group.id);
      ...
      setError("Error eliminando el grupo. Intenta de nuevo.");
    } finally {
              ...
              return (
                <div key={group.id} className="group relative">
                  ...
                  <button
                    ...
                    className={`absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-lg border border-red-100 bg-red-50 text-red-600 shadow-sm transition hover:bg-red-600 hover:text-white focus-visible:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 ${
